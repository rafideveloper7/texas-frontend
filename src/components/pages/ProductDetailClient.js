...
export default function ProductDetailClient({ product, relatedProducts }) {
  ...
  return (
    <div className="bg-gradient-to-b from-[#121212] to-[#0a0a0a] min-h-screen pt-20 md:pt-28 pb-20 font-sans text-white">
      <AnimatePresence>
        {showToast && (
          <motion.div initial={{ opacity: 0, y: 50, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, y: 50, x: "-50%" }} className="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-50 bg-gradient-to-r from-[#cc2b2b] to-[#d8a43f] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 mb-16">
          <div className="lg:w-1/2">
            <div className="relative rounded-2xl overflow-hidden bg-[#1a1a1a] border border-gray-800 aspect-[4/3]">
              <FastImage src={productImages[activeImage]} alt={product.name} fill priority sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" />
            ...
            {productImages.length > 1 && (
              <div className="flex gap-3 mt-4 overflow-x-auto pb-2">
                {productImages.map((img, idx) => (
                  <button key={idx} onClick={() => setActiveImage(idx)} className={`relative w-20 h-20 rounded-xl overflow-hidden border-2 transition-all ${activeImage === idx ? "border-[#d8a43f]" : "border-gray-800 hover:border-gray-600"}`}>
                    <FastImage src={img} alt="" fill sizes="80px" className="object-cover" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="lg:w-1/2">
            <span className="text-[#d8a43f] text-sm font-bold uppercase tracking-wider">{product.category && typeof product.category === "object" ? product.category.name : "Premium Dish"}</span>
                ...
                <Link href={`/product/${item._id}`} key={item._id}>
                  <div className="bg-gradient-to-br from-[#1a1a1a] to-[#0f0f0f] rounded-2xl overflow-hidden border border-gray-800 hover:border-[#d8a43f]/50 transition-all">
                    <div className="relative h-32 md:h-40 overflow-hidden">
                      <FastImage src={item.image?.url || item.image} alt={item.name} fill sizes="(max-width: 768px) 50vw, 25vw" className="object-cover hover:scale-110 transition duration-500" />
                    </div>
                    <div className="p-3">
